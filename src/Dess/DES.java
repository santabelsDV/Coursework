package Dess;//Copyright Popa Tiberiu 2011
...
public class DES {
    ...
    public static byte[] TripleDES_Encrypt(byte[] data, byte[][] keys) {
        byte[] tmp = encrypt(data, keys[0]);
        byte[] tmp1 = encrypt(tmp, keys[1]);
        byte[] tmp2 = encrypt(tmp1, keys[2]);
        return tmp2;
    }


    public static byte[] TripleDES_Decrypt(byte[] data, byte[][] keys) {
        byte[] tmp = decrypt(data, keys[2]);
        byte[] tmp1 = decrypt(tmp, keys[1]);
        byte[] tmp2 = decrypt(tmp1, keys[0]);
        return tmp2;
    }
